...
# -*- coding: utf-8 -*-
"""
計算コスト・ベンチマーク — INSECTS 設定 (R1-11, d=1286)

compute_cost_benchmark.py (email 相当, d=833) の多クラス版。査読要求により
実データ最大構成 d=1286 (input=33, hidden=32, classes=6) で再計測する。
d=833 より要求に忠実 (INSECTS 本実験と同一次元)。
...
PF / WSPF-A / WSPF-B について以下を計測・報告する:
  ...
  - メモリ: WSPF-A が実際に保持する行列(Woodbury: B×B)と、素朴な
    d×d 直接保持の対比 (d=1286 では ~10.6GB)

出力:
  outputs/compute_cost/
    - insects_compute_cost.txt
    - insects_compute_cost.csv
"""
...
def main():
    ...
    model = MulticlassNeuralNetModel(INPUT_DIM, HIDDEN_DIM, N_CLASSES)
    d = model.param_dim

    print("=" * 72)
    print("Compute-cost benchmark — INSECTS (R1-11)")
    print(f"  d(param)={d}, N={N_PARTICLES}, B={BATCH_SIZE}, "
          f"classes={N_CLASSES}, measure_steps={MEASURE_STEPS}")
    print("=" * 72)
    ...
    results.append(run_filter_bench(
        "WSPF-B", wspf_b, ps_grad_fn, loglik_fn, model, rng))
    results.append(run_filter_bench(
        "WSPF-A", wspf_a, ps_grad_fn, loglik_fn, model, rng))
    ...
    emit(f"  → d×d を陽に保持せず "
         f"{mem['dense_dxd_MB'] / mem['woodbury_transient_MB']:.0f}× 削減")

    # ---- 保存 ----
    txt_path = os.path.join(OUTPUT_DIR, "insects_compute_cost.txt")
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines) + "\n")

    csv_path = os.path.join(OUTPUT_DIR, "insects_compute_cost.csv")
