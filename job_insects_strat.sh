#!/bin/bash
#
# WSPF 実験ジョブ — INSECTS (abrupt, balanced) 多クラス分類 層化学習率 WSPF
...
#   層化は粒子更新/リサンプリングに作用し、報告指標は混合予測を反映する。
#   既存 configs/outputs/insects は不変更。対照 (fixed版) は outputs/insects に計算済み。
#   本ジョブは層化版 outputs/insects_strat を生成し §8.2 のアブレーション比較に用いる。
#
# 実行順 (フルスイート, job_insects.sh と同一; Oracle/勾配ノイズ解析は回帰専用のため無し):
#   1. grid_search        選択区間(先頭1250ステップ, 2スイッチ含む)で HP 選択 (F1)
...
mkdir -p logs
MAIN_LOG="logs/WSPF_insects_strat.${PBS_JOBID:-manual}.log"
exec >"$MAIN_LOG" 2>&1
...
if [ "${THIN_METHODS:-0}" = "1" ]; then
    ...
    $PY - <<'PYEOF'
import yaml
cfg = yaml.safe_load(open("configs/insects_strat.yaml"))
cfg["methods"] = [m for m in cfg["methods"] if m != "WSPF-A"]
cfg["output_dir"] = "outputs/insects_strat_thin"
with open("configs/insects_strat_thin.yaml", "w") as f:
    yaml.safe_dump(cfg, f, allow_unicode=True, sort_keys=False)
print("生成: configs/insects_strat_thin.yaml  methods=", cfg["methods"])
PYEOF
    BENCH=insects_strat_thin
fi
...
echo "INSECTS 層化学習率 WSPF 実験が完了しました。"
echo "対照 (fixed) は outputs/insects、層化版は outputs/${BENCH} を参照。"
